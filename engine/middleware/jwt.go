...
type (
	...
	JWTConfig struct {
		...
		ParseTokenFunc func(auth string, c engine.Context) (interface{}, error)
	}

	// JWTSuccessHandler defines a function which is executed for a valid token.
	JWTSuccessHandler func(ctx engine.Context, token interface{}) error

	// JWTErrorHandler defines a function which is executed for an invalid token.
	JWTErrorHandler func(err error) error

	// JWTErrorHandlerWithContext is almost identical to JWTErrorHandler, but it's passed the current context.
	JWTErrorHandlerWithContext func(err error, c engine.Context) error
)
...
// Errors
var (
	ErrJWTMissing = engine.NewHTTPError(http.StatusBadRequest, "request was made with invalid credentials")
	ErrJWTInvalid = engine.NewHTTPError(http.StatusUnauthorized, "invalid or expired authentication token")
)
...
var (
	...
	DefaultJWTConfig = JWTConfig{
		...
		TokenLookup:      "header:" + engine.HeaderAuthorization,
...
func JWT(key interface{}) engine.MiddlewareFunc {
	c := DefaultJWTConfig
	c.SigningKey = key
...
// See: `JWT()`.
func JWTWithConfig(config JWTConfig) engine.MiddlewareFunc {
	// Defaults
	if config.Skipper == nil {
		config.Skipper = DefaultJWTConfig.Skipper
	}
	if config.SigningKey == nil && len(config.SigningKeys) == 0 && config.KeyFunc == nil && config.ParseTokenFunc == nil {
		panic("engine: jwt middleware requires signing key")
	}
	...
	return func(next engine.HandlerFunc) engine.HandlerFunc {
		return func(c engine.Context) interface{} {
			...
			for _, extractor := range extractors {
				...
				for _, auth := range auths {
					...
					if config.SuccessHandler != nil {
						handlerErr := config.SuccessHandler(c, token)
						if handlerErr != nil {
							lastTokenErr = handlerErr
							break
						}
					}
					return next(c)
			...
			if lastTokenErr != nil {
				_, ok := lastTokenErr.(*engine.HTTPError)
				if ok {
					return lastTokenErr
				}

				return &engine.HTTPError{
...
func (config *JWTConfig) defaultParseToken(auth string, c engine.Context) (interface{}, error) {
	token := new(jwt.Token)
	var err error
	// Issue #647, #656
