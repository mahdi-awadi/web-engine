...
func (r *Router) Add(method, path string, h HandlerFunc) {
	...
	if h == nil && r.engine.Logger != nil {
		// FIXME: in future we should return error
		r.engine.Logger.Errorf("Adding route without handler function: %v:%v", method, path)
	}
...
func (r *Router) insert(method, path string, h HandlerFunc, t kind, ppath string, pnames []string) {
	// Adjust max param
	paramLen := len(pnames)
	if *r.engine.maxParam < paramLen {
		*r.engine.maxParam = paramLen
	}
...
func optionsMethodHandler(allowMethods string) func(c Context) interface{} {
...
func (r *Router) Find(method, path string, c Context) {
	...
	for {
		...
		if search == "" && currentNode.isHandler {
			// check if current node has handler registered for engine method we are looking for. we store currentNode as
	...
	if matchedHandler != nil {
		...
		// so we can send engine.StatusMethodNotAllowed (405) instead of engine.StatusNotFound (404)
