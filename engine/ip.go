...
/**
By: https://github.com/tmshn (See: https://github.com/labstack/engine/pull/1478 , https://github.com/labstack/echox/pull/134 )
Source: https://engine.labstack.com/guide/ip-address/

IP address plays fundamental role in HTTP; it's used for access control, auditing, geo-based access analysis and more.
Engine provides handy method [`Context#RealIP()`](https://godoc.org/github.com/labstack/engine#Context) for that.
...
In Engine, this can be done by configuring `Engine#IPExtractor` appropriately.
This guides show you why and how.

> Note: if you dont' set `Engine#IPExtractor` explicitly, Engine fallback to legacy behavior, which is not a good choice.
...
In this case, use `engine.ExtractIPDirect()`.

```go
e.IPExtractor = engine.ExtractIPDirect()
```

## Case 2. Base proxies using `X-Forwarded-For` header
...
In Engine, use `ExtractIPFromXFFHeader(...TrustOption)`.

```go
e.IPExtractor = engine.ExtractIPFromXFFHeader()
```
...
To control this behavior, use [`TrustOption`](https://godoc.org/github.com/labstack/engine#TrustOption)s.
...
- Ref: https://godoc.org/github.com/labstack/engine#TrustOption

## Case 3. Base proxies using `X-Real-IP` header
...
```go
e.IPExtractor = engine.ExtractIPFromRealIPHeader()
```
...
In default behavior, Engine sees all of first XFF header, X-Real-IP header and IP from network layer.
...
// See https://engine.labstack.com/guide/ip-address for more details.
