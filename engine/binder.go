...
/**
  ...
  ```go
  var length int64
  err := engine.QueryParamsBinder(c).Int64("length", &length).BindError()
  ```
...
// BindingError represents an error that occurred while binding request messages.
...
// Note:
//  * time.Time{} (param is empty) and time.Unix(0,0) (param = "0") are not equal
//  * Javascript's Number type only has about 53 bits of precision (Number.MAX_SAFE_INTEGER = 9007199254740991). Duration it to 1609180603123456789 in example.
